...
def fmin(objective_function, x0, sigma0,
    ...
    ``restarts=0``
        ...
        opts['maxfevals']))`` and neither the ``'ftarget'`` nor the
        ``termination_callback`` option was triggered; to restart from
        different points (recommended), pass ``x0`` as a `callable`; see
        also parameter ``bipop``.
    ``restart_from_best=False``
