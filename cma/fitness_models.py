...
class ModelInjectionCallback_1:
    """callback class for `cma.fmin2`, first version.

    Inject the model optimum and possibly move ``es.mean`` closer to it
    and decrease ``es.sigma``.

    LATEST CHANGE: inject model.xopt only if
          ||model.xopt - es.mean|| > threshold, else decrease es.sigma

    WONTFIX TODO: Move es.mean no further than by 1.5 * E ||0, C|| (move also if
          ||model.xopt - es.mean|| is large?!).

    WONTFIX TODO: update docs, no model update, only injection.

    ::

        >> import numpy as np
        >> import cma
        >> # from cma.fitness_models
        >> # from . import ModelInjectionCallback, Model, ModelUpdateCallback
        >> fitness = cma.fitness_transformations.Rotated(cma.ff.elli)
        >> model = Model()
        >> inmodel = ModelInjectionCallback(fitness, model)
        >> upmodel = ModelUpdateCallback(model)
        >> xopt, es = cma.fmin2(fitness, 4 * [1], 1,
        ..                      {'ftarget':1e-6, 'verbose':-9},
        ..                      callback=[upmodel, inmodel])
        >> assert es.best.f < 1e-6

        >> R = fitness[1].dicMatrices[4]
        >> H = np.dot(np.dot(R.T, np.diag([1, 1e2, 1e4, 1e6])), R)
        >> assert np.all(np.isclose(H, model.hessian))

    """
    def __init__(self, fitness, model,
                 wopt=0, sigma_decrease=1, comparison_index=None, min_data=1):
        """fitness: `callable`, model: `Model`

        wopt moves es.mean towards model.xopt (inactivated),
        sigma_decrease moves
        es.sigma towards zero. `comparison_index is None` means
        unconditional injection if `min_data` data were added to the model.

        `min_data`: number of additional data required to act again
        `comparison_index`: fitness index to compare the model optimum
        with for possible injection, `None` means unconditional injection.

        WONTFIX TODO: retry simple unconditional injection, which means min_data=0
        (recheck the code) and norm_threshold.

        WONTFIX TODO: unconditional move towards xopt using wopt does not make
        sense!

        WONTFIX TODO: the evaluation to make the injection decision here has no
        other effect than changing the population size by +-1.
        ...
        self.min_data = min_data  # to invoke injection
        self.last_count = 0

    def __call__(self, es):
        """inject if model was recently updated and `self.xopt` is better

        than `es.fit.fit[0]`.
        """
        norm_threshold = 0.5  # to invoke mean shift and sigma decrease
        sqrtN = np.sqrt(es.N)

        if self.model.count < self.last_count + self.min_data:
            return
        self.last_count = self.model.count
        if 11 < 3 and es.countiter > 1:
            F_model = [self.model.eval(x)
                       for x in [self.model.xopt] + es.pop_sorted]
            print(' '.join('%.1e' % _f for _f in F_model))
        # self.model.add_data(es.pop_sorted, es.fit.fit)
        # self.model.set_xoffset(-es.mean)
        if 11 < 3:
            F_model = [self.model.eval(x)
                       for x in [self.model.xopt] + es.pop_sorted]
            print(' '.join('%.1e' % _f for _f in F_model))
        ...
        if self.comparison_index is not None:
            # CAVEAT: this makes one evaluation here and another one if
            # xopt was injected, which doesn't make too much sense
            # compared to evaluating the potential injection only
            es.countevals += 1
            f_model = self.fitness(self.model.xopt)
        else:
            f_model = -np.inf
        if self.comparison_index is None or f_model < es.fit.fit[self.comparison_index]:
            if es.mahalanobis_norm(self.model.xopt - es.mean) / sqrtN > norm_threshold:
                es.inject([self.model.xopt])
            else:
                es.sigma *= self.sigma_decrease
            if self.wopt > 0:
                dx = self.wopt * (self.model.xopt - es.mean)
                norm = es.mahalanobis_norm(dx)
                if norm > 1.5 * sqrtN:
                    dx *= 1.5 * sqrtN / norm
                es.mean += dx

        # this can be incommented
        if 11 < 3 and es.mahalanobis_norm(self.model._xoffset - es.mean) > 10 * sqrtN:
            self.model.set_xoffset(es.mean)

class ModelUpdateCallback_1:
    """callback class for `cma.fmin2`.

    WONTFIX TODO: update docs: no injection, only update.
    WONTFIX TODO: why is this needed anyways?
    Adding the true f-values to the model is done with the (parallel)
    evaluation. It would only be needed if state variables like
    mean or sigma are changed or the model optimum is injected.
    ...
    ::
        ...
        >> import cma
        >> # from cma.fitness_models
        >> # from . import ModelUpdateCallback, Model
        >> fitness = cma.fitness_transformations.Rotated(cma.ff.elli)
        >> cbmodel = ModelUpdateCallback(Model(), fitness)
        >> xopt, es = cma.fmin2(fitness, 4 * [1], 1,
        ..                      {'ftarget':1e-6, 'verbose':-9},
        ..                      callback=cbmodel)
        >> assert es.best.f < 1e-6
        >> R = fitness[1].dicMatrices[4]
        >> H = np.dot(np.dot(R.T, np.diag([1, 1e2, 1e4, 1e6])), R)
        >> assert np.all(np.isclose(H, cbmodel.model.hessian))

    """
    def __init__(self, model, fitness=None):
        """fitness: `callable`, model: `Model`

        wopt moves es.mean towards model.xopt, sigma_decrease moves
        es.sigma towards zero.
    ...
    def __call__(self, es):
        """add all solutions of ``es.pop_sorted`` to the model.
       """
        if 11 < 3 and es.countiter > 1:
            F_model = [self.model.eval(x)
                       for x in [self.model.xopt] + es.pop_sorted]
            print(' '.join('%.1e' % _f for _f in F_model))
            if F_model[0] > F_model[1]:
                self.done = True
                return
        self.model.add_data(es.pop_sorted[::-1], es.fit.fit[::-1])
        # self.model.set_xoffset(-es.mean)
        if 11 < 3:
            F_model = [self.model.eval(x)
                       for x in [self.model.xopt] + es.pop_sorted]
...
class ModelLogger:
    default_filename = '_model-log'
    def __init__(self):
        self.filename = ModelLogger.default_filename
        self.count = 0
        self.current_data = []
    def add(self, data):
        self.current_data += data
    def add(self, model):
        ...
        s = str(10**model.tau.tau)
        evals = np.linalg.eigvals(model.hessian)
        s += ' ' + ' '.join("%9.2e" % val for val in evals)
        if self.current_data:
            s += ' ' + ' '.join("%9.2e" % val for val in self.current_data)
            self.current_data = []
        mode = 'at' if self.count else 'wt'
        with open(self.filename, mode) as file_:
            file_.write(s + '\n')
        self.count += 1
    def load(self):
    ...
    def plot(self, plot=None):
        ...
        n = len(self.data)
        m = len(self.data[0])
        color=iter(pyplot.cm.winter_r(np.linspace(0.15, 1, m - 1)))
        plt.gca().clear()
        for i in range(m - 1):
            plot(range(n), self.data[:, 1 + i], color=next(color))
        plot(range(n), self.data[:,0], 'r')
        return self
